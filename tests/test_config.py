...
import ninfer
from stack.config import MANIFEST, MODEL_PROFILES, RUNTIME_PROFILES, runtime_env_values, spec_values, validate_manifest, validate_spec
...
class ConfigurationTests(unittest.TestCase):
    def test_manifest_profiles_context_and_generated_docs(self):
        ...
        for profile in RUNTIME_PROFILES.values():
            ...
            self.assertLess(int(values['HERMES_COMPRESSION_THRESHOLD_TOKENS']), int(values['NINFER_CONTEXT_LENGTH']) - 8191)
    ...
    def test_spec_modes_and_capability_gate(self):
        ...
        for backend, count in [('mtp',6),('dflash2',16),('dflash2',0),('unknown',3)]:
    ...
    def test_corrupt_model_and_manifest_mismatch(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            model=replace(MODEL_PROFILES['stock'],expected_bytes=5,sha256=hashlib.sha256(b'valid').hexdigest())
    ...
    def test_profile_composition_maps_memory_and_spec(self):
        ...
        for key in ['NINFER_CONTEXT_LENGTH','NINFER_KV_CAPACITY','NINFER_DEVICE_STATE_SLOTS','NINFER_HOST_STATE_SLOTS','NINFER_HOST_KV_MIB','NINFER_SPEC_BACKEND','NINFER_DRAFT_TOKENS']:
