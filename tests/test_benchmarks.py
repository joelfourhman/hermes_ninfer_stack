...
class BenchmarkTests(unittest.TestCase):
    def test_coding_fixture_requires_two_real_edit_test_cycles(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual([event['failed'] for event in result['tools'] if event['tool']=='run_tests'],[True,False])
    ...
    def test_tool_boundaries(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('error',workspace.execute('write_file',{'path':'test_ledger.py','content':'pass'}))
    ...
    def test_stream_reasoning_tool_fragments_empty_usage_and_roundtrip(self):
            ...
            def do_POST(self):
                ...
                events=[{'choices':[{'delta':{'role':'assistant'}}]},
                    ...
                    {'choices':[],'usage':{'prompt_tokens':100,'completion_tokens':10,'prompt_tokens_details':{'cached_tokens':80}},
                     'timings':{'cache_n':80,'prompt_n':20,'prompt_ms':50,'predicted_ms':20,'predicted_per_second':450}}]
                for event in events:
        ...
        try:
            ...
            self.assertEqual(json.loads(first.message['tool_calls'][0]['function']['arguments']),{'path':'README.md'})
            client.complete([{'role':'user','content':'Read file'},first.message,{'role':'tool','tool_call_id':'call_1','content':'ok'}])
