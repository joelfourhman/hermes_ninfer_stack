...
The model download is 20.02 GiB. It needs at least 24 GiB free on the drive
where you clone this repository. Docker also needs separate free space for its
Linux image and build files, so 24 GiB is not the complete disk requirement.
Close games and other GPU-heavy applications before setup so NInfer can use the
5090's memory.
...
2. At **Download the model now? [Y/n]**, just press Enter. Leave Command Prompt
   open while the model downloads, verifies, builds, and loads. The first run
   can take a while and may appear quiet during large downloads or builds.
...
- The checksum-pinned Qwen3.8-27B NVFP4 model workflow.
...
- At least 24 GiB of free disk space for the approximately 20.02 GiB model.
...
settings, asks before downloading the 20.02 GiB model, verifies the download,
builds and starts NInfer, waits for the model to become ready, and then offers
the official stock Hermes Desktop installation and configuration. It is safe
to rerun after a cancelled or interrupted download.
...
| `NINFER_MODEL_FILE` | `qwen3_8_27b_nvfp4.ninfer` | Artifact mounted read-only |
| `NINFER_MODEL_ID` | `qwen-local` | API alias configured in Hermes |
| `NINFER_CONTEXT_LENGTH` | `65536` | Per-request sequence ceiling |
| `NINFER_KV_CAPACITY` | `65536` | Total resident KV-token budget |
...
├── docker-compose.yml         NInfer service and model downloader profile
├── .env.example               Supported non-secret defaults
├── ninfer/                    Pinned NInfer source submodule
├── model-downloader/          Isolated resumable artifact downloader
