...
class Client:
    ...
    def complete(self, messages: list[dict], *, tools: list[dict] | None = None,
                 max_tokens: int = 1024, seed: int = 1, thinking: bool = False) -> Completion:
        payload = dict(model=self.model, messages=messages, max_tokens=max_tokens,
                       temperature=0, seed=seed, enable_thinking=thinking, stream=True,
        ...
        with self.post('/chat/completions', payload) as response:
            ...
            for raw in response:
                ...
                for choice in event.get('choices') or []:
                    ...
                    visible = delta.get('content') or delta.get('reasoning_content') or delta.get('tool_calls')
                    if visible:
        ...
        metrics = {
            'request_id': request_id, 'input_tokens': prompt,
            'generated_tokens': usage.get('completion_tokens'), 'cached_prefix_tokens': cached,
            'fresh_prefill_tokens': fresh, 'context_tokens': prompt,
            'ttft_seconds_client': first - start if first is not None else None,
            'request_seconds_client': elapsed,
            'prompt_seconds_native': timings.get('prompt_ms', 0) / 1000 if 'prompt_ms' in timings else None,
            'decode_seconds_native': timings.get('predicted_ms', 0) / 1000 if 'predicted_ms' in timings else None,
            ...
            'timings_native': timings, 'usage_native': usage,
