...
def register_commands(sub) -> None:
    ...
    bench = sub.add_parser("bench-agent", help="run repeatable coding/research/session agent workloads")
    ...
    bench.add_argument("--context-kib", type=int, default=64, help="fixture text KiB; actual token counts are recorded")
    ...
    bench.add_argument("--baseline", action="store_true", help="measure original image with verified baseline revision")
    bench.add_argument("--smoke", action="store_true", help="offline coding fixture smoke; no performance claims")
    ...
    metrics = sub.add_parser("observe", help="snapshot hardware and native NInfer structured counters")
