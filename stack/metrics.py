...
def command_json(command: list[str]) -> object:
    return json.loads(subprocess.check_output(command, text=True, stderr=subprocess.PIPE, timeout=60))
...
def runtime_metadata(helper, *, baseline: bool = False, hash_model: bool = True) -> dict:
    ...
    labels = command_json([helper.docker_executable() or 'docker', 'image', 'inspect', info['Image']])[0]['Config'].get('Labels') or {}
    ...
    flags = {args[i]: args[i+1] for i in range(len(args)-1) if args[i].startswith('--') and not args[i+1].startswith('--')}
    mapping = {'--max-context':'NINFER_CONTEXT_LENGTH', '--kv-capacity':'NINFER_KV_CAPACITY',
               '--max-concurrency':'NINFER_MAX_CONCURRENCY', '--kv-dtype':'NINFER_KV_DTYPE',
               '--device-state-slots':'NINFER_DEVICE_STATE_SLOTS', '--host-state-slots':'NINFER_HOST_STATE_SLOTS',
               '--host-kv-mib':'NINFER_HOST_KV_MIB', '--spec':'NINFER_SPEC_BACKEND', '--draft-tokens':'NINFER_DRAFT_TOKENS',
    ...
    for flag, key in mapping.items():
        if flags.get(flag) != values.get(key, defaults.get(key)):
            raise ValueError(f'Running {flag} differs from configured {key}; recreate before benchmarking')
    if '/models/' + values['NINFER_MODEL_FILE'] not in args:
        raise ValueError('Running model filename differs from configured artifact')
    digest = verify_model(helper.ROOT/'models'/values['NINFER_MODEL_FILE'],values['NINFER_MODEL_PROFILE']) if hash_model else None
    return {'container_id': container, 'image_id': info['Image'], 'ninfer_revision': revision,
            'source_expected': NINFER_COMMIT, 'baseline': baseline, 'model_sha256_verified': digest,
            'config': {key: value for key,value in values.items() if key.startswith('NINFER_') and key != 'NINFER_API_KEY'},
            'runtime_flags': {key:flags.get(key) for key in mapping}, 'hermes_session_id': None}
...
def parse_native_logs(text: str) -> list[dict]:
    ...
    for line in text.splitlines():
        ...
        try:
            ...
            continue
        if record.get('event') in {'server_start','request_done','throughput','request_error','request_rejected','request_start'}:
...
class Sampler:
    ...
    def _run(self):
        while not self.stop.is_set():
            row={'time_unix':time.time(),'system_ram_used_mib':memory_used_mib(), 'gpu_utilization_percent':None,'vram_used_mib':None}
            if self.command:
                try:
                    result=subprocess.run([self.command,'-i',self.gpu,'--query-gpu=utilization.gpu,memory.used','--format=csv,noheader,nounits'],capture_output=True,text=True,timeout=5,check=False)
...
def observe(args) -> None:
    ...
    raw=helper.compose('logs','--no-color','--tail',str(args.lines),'ninfer',capture=True).stdout
    ...
    result={'metadata':metadata,'hardware':sampler.samples[-1] if sampler.samples else None,
            'latest_native_events':latest,'job':None,
