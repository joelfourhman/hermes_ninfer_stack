...
def run_workload(client: Client, workspace: Workspace, workload: str, *, max_turns: int = 24,
                 max_tokens: int = 1024, session_turns: int = 12, thinking: bool = False,
                 compression_tokens: int = 90000) -> dict:
    ...
    requests=[]; failures=[]; compressions=[]; continuations=0
    answer=''; finished=False
    for turn in range(max_turns):
        if requests and (requests[-1].get('input_tokens') or 0)>compression_tokens and workload=='long-session':
            summary=client.complete(messages+[{'role':'user','content':'Summarize the essential findings for continuation. No tools.'}],max_tokens=512)
            ...
            messages=[messages[0],{'role':'user','content':'Retained session findings: '+(summary.message.get('content') or '')}]
        try:
            response=client.complete(messages,tools=TOOLS,max_tokens=max_tokens,thinking=thinking)
        ...
        if workload=='long-session' and continuations<session_turns-1:
            ...
            source=(workspace.root/f'source-{continuations%4}.txt').read_text(encoding='utf-8')
            messages.append({'role':'user','content':f'Epoch {continuations}: inspect this new evidence and give one concise finding.\n{source}'})
            continue
        finished=True
        break
    validation=None
    if workload=='coding':
        ...
        success=finished and validation['passed'] and sum(e['tool']=='write_file' for e in workspace.events)>=2 and sum(e['tool']=='run_tests' for e in workspace.events)>=2
    elif workload=='research':
        success=finished and all(f'FIXTURE-{i}' in answer for i in range(4)) and sum(e['tool']=='read_file' for e in workspace.events)>=4
    else:
        success=finished and continuations==session_turns-1
    elapsed=time.perf_counter()-start
    return {'workload':workload,'driver':'bounded-agent','success':success,'wall_seconds':elapsed,
            ...
            'requests':requests,'tools':workspace.events,'compression_events':compressions,
            'failures':failures,'retries':0,'validation':validation,
            'answer':answer,'system_tools_sha256':hashlib.sha256(json.dumps([SYSTEM,TOOLS],separators=(',',':')).encode()).hexdigest(),
...
def benchmark(args) -> None:
    ...
    helper=_helper()
    if not 1<=args.runs<=20 or not 1<=args.max_turns<=200 or not 32<=args.max_tokens<=8192 or not 1<=args.context_kib<=16384 or not 2<=args.session_turns<=100:
        raise ValueError('Benchmark limits: runs 1..20, turns 1..200, output 32..8192, context KiB 1..16384, session turns 2..100')
    if args.smoke:
        ...
        client=Client(helper.ninfer_endpoint(values),values['NINFER_API_KEY'],values['NINFER_MODEL_ID'],args.timeout)
    result_dir=Path(args.output).resolve() if args.output else helper.ROOT/'benchmarks'/('agent-'+time.strftime('%Y%m%dT%H%M%S')+'-'+uuid.uuid4().hex[:6])
    result_dir.mkdir(parents=True,exist_ok=False)
    result={'schema_version':1,'fixture_version':WORKLOAD_VERSION,'workload':args.workload,'driver':'mock' if args.smoke else args.driver,
            'parameters':{k:v for k,v in vars(args).items() if k not in {'func','command'}},
            'environment':metadata,'runs':[],'resource_samples':[], 'native_records':[],
            'started_at_unix':time.time(), 'metric_scope':'Model request time includes transport/queueing; parallel sums overlap. Native interval cache counters are not attributed to a single request.'}
    ...
    def one(index, lane=0):
        ...
        return run_workload(client,workspace,kind,max_turns=args.max_turns,max_tokens=args.max_tokens,
                            session_turns=args.session_turns,thinking=args.thinking,
                            compression_tokens=int(values['HERMES_COMPRESSION_THRESHOLD_TOKENS']))
    sampler=Sampler(None if args.smoke else helper.nvidia_smi_executable(),values.get('NINFER_GPU_DEVICE','0'))
    try:
        with sampler:
            for index in range(args.runs):
                ...
                if args.workload=='parallel':
                    ...
                    row={'workload':'parallel','success':all(r['success'] for r in lanes),'wall_seconds':time.perf_counter()-start,'lanes':lanes}
                else:
                    row=one(index)
                result['runs'].append(row)
                (result_dir/'results.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8')
                print(f"Run {index+1}/{args.runs}: {'PASS' if row['success'] else 'FAIL'}, {row['wall_seconds']:.2f}s")
    finally:
        result['resource_samples']=sampler.samples
        if not args.smoke:
            logs=helper.compose('logs','--no-color','--since',started,'ninfer',capture=True,check=False)
...
def compare(args) -> None:
    ...
    for result in results:
        ...
        if any(result.get(key)!=first.get(key) for key in ('fixture_version','workload','driver')):
            raise ValueError('Comparison requires identical fixture version, workload and driver')
        for key in ('max_turns','max_tokens','context_kib','session_turns','thinking'):
    ...
    for result in results:
        ...
        print(f"| {config['NINFER_RUNTIME_PROFILE']} / {config.get('NINFER_SPEC_BACKEND','mtp')}-{config.get('NINFER_DRAFT_TOKENS','3')} | {len(passed)}/{len(result['runs'])} | {median} |")
