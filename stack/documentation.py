...
def profile_table() -> str:
    rows = ["| Profile | Context tokens | Shared KV tokens | Lanes | Device / host slots | Host KV MiB | Compression tokens | Turns |",
            "|---|---:|---:|---:|---:|---:|---:|---:|"]
    for p in RUNTIME_PROFILES.values():
        rows.append(f"| `{p.key}` | {p.context_length:,} | {p.kv_capacity:,} | {p.max_concurrency} | {p.device_state_slots} / {p.host_state_slots} | {p.host_kv_mib:,} | {p.compression_threshold_tokens:,} | {p.max_turns} |")
    return "\n".join(rows)


def generated_reference() -> str:
    rows = ["# Generated configuration reference", "", "Generated from `stack/manifest.json`; edit the manifest, then run `python ninfer.py docs`.", "",
            f"NInfer source/image revision: `{NINFER_COMMIT}`.",
            f"CUDA image base: `{MANIFEST['ninfer']['cuda_base']}`.", "", profile_table(), "",
    ...
    for m in MODEL_PROFILES.values():
        rows += [f"## {m.key}", "", f"{m.label}. {m.quantization}.", "",
                 f"- Repository: `{m.repository}`", f"- Revision: `{m.revision}`",
                 f"- Remote artifact: `{m.source_filename}`", f"- Local artifact: `{m.filename}`",
                 f"- Bytes: {m.expected_bytes}", f"- SHA-256: `{m.sha256}`",
...
def generate(check: bool = False) -> None:
    ...
    if check:
        if not target.exists() or target.read_text(encoding="utf-8") != expected:
            raise ValueError("Generated configuration documentation is stale; run python ninfer.py docs")
    else:
