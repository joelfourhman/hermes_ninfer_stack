...
TOOLS = [tool('list_files','List fixture files'),
         tool('read_file','Read one fixture file',{'path':{'type':'string'}},['path']),
         tool('write_file','Replace ledger.py with corrected Python',{'path':{'type':'string'},'content':{'type':'string'}},['path','content']),
...
class Workspace:
    def __init__(self, root: Path, *, context_kib: int = 64):
        ...
        for index in range(4):
            lines=[f'Record {index}-{n:06d}: module_{n%97} validates tenant scope before cache lookup; risk class {n%11}; evidence tag {n*7919+index}.' for n in range(max(1,context_kib*1024//4//120))]
            lines += [f'Finding {index}: service shard-{index} has boundary token FIXTURE-{index}; remediation is tenant-scoped cache keys.']
    ...
    def execute(self, name: str, arguments: dict) -> str:
        ...
        self.events.append({'tool':name,'duration_seconds':time.perf_counter()-start,'failed':failed})
        return result
    ...
    def test(self) -> dict:
        ...
        result=subprocess.run([sys.executable,'-I','-c',
            'import sys,unittest; sys.path.insert(0, sys.argv[1]); suite=unittest.defaultTestLoader.discover(sys.argv[1]); r=unittest.TextTestRunner(verbosity=1).run(suite); sys.exit(not r.wasSuccessful())',str(self.root)],
            cwd=self.root,capture_output=True,text=True,timeout=20,check=False)
        return {'passed':result.returncode==0,'returncode':result.returncode,'output':(result.stdout+result.stderr)[-12000:]}
...
def validate_fixture_code(text: str) -> None:
    ...
    forbidden=(ast.Import,ast.ImportFrom,ast.Attribute,ast.While,ast.For,ast.AsyncFunctionDef,ast.ClassDef,ast.Lambda,ast.With,ast.Try,ast.Global,ast.Nonlocal)
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node,ast.FunctionDef) and (node.decorator_list or node.args.defaults or node.args.kw_defaults):
