...
def verify_source(root: Path, *, check_index: bool = False) -> str:
    ...
    if actual != NINFER_COMMIT:
        raise ValueError(f"NInfer submodule HEAD is {actual}; manifest expects {NINFER_COMMIT}. Run setup.")
    ...
    if check_index:
        ...
        if entry[:2] != ["160000", NINFER_COMMIT]:
            raise ValueError("NInfer gitlink differs from manifest; stage the audited submodule update")
...
def verify_model(path: Path, key: str) -> str:
    ...
    if path.name != model.filename or not path.is_file() or path.stat().st_size != model.expected_bytes:
    ...
    if checksum.hexdigest() != model.sha256:
        raise ValueError(f"Model SHA-256 differs from {key} manifest; existing bytes were preserved")
